...
def activities(country_code, iati_identifier):
    ...
    sectors = abprojects.DAC_codes_cc_mappable()
...
def activity_add_sector(country_code, iati_identifier):
    ...
    sector_data = abprojects.sector(sector_code)

    if abprojects.add_sector_to_project(sector_code, iati_identifier, percentage):
...
def activity_delete_sector(country_code, iati_identifier):
    ...
    result = abprojects.delete_sector_from_project(sector_code, iati_identifier)
    if result:
...
def activity_restore_sector(country_code, iati_identifier):
    sector_code = request.form['sector_code']
    iati_identifier = iati_identifier
    if abprojects.restore_sector_to_project(sector_code, iati_identifier):
